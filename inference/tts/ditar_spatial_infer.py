...
class DiTARSInfer(DiTARBuildModelMixinS):
    ...
    def preprocess(self, test_id):
        ...
        global_clip_embedding = torch.from_numpy(global_clip_embedding).to(self.device, dtype=torch.float32)
        global_clip_embedding = global_clip_embedding.unsqueeze(0)
        
        self.metadata = {
            'target_lat_len': int(vid_len/4*self.frame_rate),
        }
        
        return video_feats, v_mask, gt, global_clip_embedding
        
    
    @torch.no_grad()
    def forward(self, test_id):

        video_feats, v_mask, gt, global_clip_embedding = self.preprocess(test_id)
        inputs = {
        ...
        latent_mean = np.load('/data/leike/spatial/YT-Ambigen/stable_latents_stats_10/mean.npy')
        latent_std = np.load('/data/leike/spatial/YT-Ambigen/stable_latents_stats_10/std.npy')
...
if __name__ == '__main__':
    ...
    SAVE_DIR = os.path.join('results', timestamp+'_ditar_v2')
