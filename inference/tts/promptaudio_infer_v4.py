...
from modules.tts.scriptspeech.build_model_utils import DiTBuildModelMixinV4
...
def build_audio_mask_from_ids(input_ids: torch.LongTensor,
    ...
    输出：
      - mask: [B, T] LongTensor（取值 0/1/2），与 attention_mask 同形状
    """
    device = input_ids.device

    # 1) 获取标签的 id 序列（支持多 token）
    ...
    for b in range(B):
        ...
        # 2) 定位每种标签的起止位置
        s1_starts = _to_int_list(_find_pattern_starts(seq, S1_OPEN))
        s1_ends   = _to_int_list(_find_pattern_starts(seq, S1_CLOSE))
        au_starts = _to_int_list(_find_pattern_starts(seq, AU_OPEN))
        au_ends   = _to_int_list(_find_pattern_starts(seq, AU_CLOSE))

        # 3) 将“起始标签右侧到结束标签左侧”的内容段赋值
        def set_between(starts, ends, open_len, value):
            i = j = 0
            Ls, Le = len(starts), len(ends)
            while i < Ls and j < Le:
                s = starts[i]
                # 找到第一个 > s 的结束
                while j < Le and ends[j] <= s:
                    j += 1
                if j >= Le:
                    break
                e = ends[j]
                left  = s + open_len    # 排除起始标签本身
                right = e               # 排除结束标签本身
                if left < right:
        ...
        # 注意：后写覆盖。若重叠，<Audio> 将覆盖为 2
        set_between(s1_starts, s1_ends, S1_OPEN.numel(), value=1)
        set_between(au_starts, au_ends, AU_OPEN.numel(), value=2)

    # 4) 去除 padding（padding 位置强制 0）
...
class MegaTTS3DiTInfer(DiTBuildModelMixinV4):
    ''' 这里指定所用ckpt的路径 '''
    def __init__(
            self, 
            device=None,
    ...
    def _build_caption_mark_from_ids(
        self,
        input_ids: torch.Tensor,
        attention_mask: torch.Tensor
    ) -> torch.Tensor:
        """
        新版：完全基于 token 序列做子串匹配，不再依赖缓存的特殊 id，
        也不区分 GPROMPT / TAG / BGM，只输出：
          0 = 其它/标签本身/填充
          1 = <S1>…</S1> 内的 token
          2 = <Audio>…</Audio> 内的 token
        """
        # 如果你的 caption 里可能有小写 <s1> / <audio>，可以在喂 tokenizer 前
        # 继续用 self._norm_caption_tags 做大小写归一。
        mark = build_audio_mask_from_ids(
            input_ids=input_ids,
        ...
        with Timer('DiT target speech generation', enable=profile):
            ...
            if is_prompttts:
                def run_caption_encoder_prompttts(captions, device):
                    caps = [self._norm_caption_tags(c) for c in captions]
                    inputs = self.caption_tokenizer(
                        caps,
                        padding=True,
                        return_tensors="pt",
                    ...
                    caption_text_mark = self._build_caption_mark_from_ids(
                        input_ids=input_ids,
                        attention_mask=attention_masks
                    )
            ...
            if prompttts_kwargs.get('ref_dur_only', False):
                ...
                if hparams.get('use_sparse_dur', False):
                    dur_pred_cat = torch.cat([dur_ref, dur_pred], dim=1)
                    mel2ph_sparse = compute_mel2aug_from_dur(
                        dur_pred_cat.int().squeeze().cpu().numpy().tolist(),
            ...
            # 这里依据 latent 时间长度构造 tgt_len（总是与 lat 对齐）
            tgt_len = torch.full(
                (lat.size(0),),
                lat.size(1),
                dtype=torch.long,
            ...
            inputs = {
                ...
                "vad_mask": vad_mask,
                "tgt_len": tgt_len,  # 关键：给 Diffusion.inference 用
            }
    ...
    def _forward_single(self, resource_context, input_text, time_step, w_all=1.0, w_txt=1.0, w_cap=1.0, w_ref=1.0, seq_cfg_w=None,
        ...
        """
        单条输入版本（与您原先的 forward 等价），返回一个 MegaTTS3Output。
        """
        device = self.device
    ...
    def forward(self, resource_context, input_text, time_step, w_all=1.0, w_txt=1.0, w_cap=1.0, w_ref=1.0, seq_cfg_w=None,
        ...
        global_prompt / local_prompt 可为 str（自动广播）或与 input_text 等长的 List[str]/List[None]。
        """
        ...
        if isinstance(input_text, list):
            n = len(input_text)
            gp_list = _broadcast(global_prompt, n, "global_prompt")
            lp_list = _broadcast(local_prompt, n, "local_prompt")
            ...
            for i in range(n):
                out_i = self._forward_single(
                    ...
                    global_prompt=gp_list[i], local_prompt=lp_list[i],
        ...
        return self._forward_single(
            ...
            num_parallel_workers=num_parallel_workers, use_sa_frontend=use_sa_frontend,
            chunk_num_words_zh=chunk_num_words_zh, chunk_num_words_en=chunk_num_words_en,
            global_prompt=global_prompt, local_prompt=local_prompt,
            prompttts_kwargs=prompttts_kwargs, **kwargs
        )
...
if __name__ == '__main__':
    ...
    # dit_exp_name = 'checkpoints/251124_promptaudio_zero_new'
    dit_exp_name = 'checkpoints/251125_promptaudio_zero_voxbox'
    ...
    scenes = [
        # 1. teenage rebellion
        {
            "name": "teenage_rebellion",
            "subject": (
                "Subject1: A woman with shoulder-length light brown hair in a close-up shot, "
                "wearing a light-colored patterned shirt in a warmly lit, cozy room with wooden shelves behind her "
                "and a blurred figure on the right suggesting an intimate conversation."
            ),
            "narration_en": (
                "The overall environment sound is quiet. The woman's expression changes subtly from a slight smile "
                "to a more serious look as her eyebrows raise slightly and she opens her mouth to speak. "
                "She looks at the person off-screen and says "
            ...
            "text_en": """<S1>I think our not-so-normal son might be going through some classic teenage rebellion.</S1>""",
            "text_zh": """<S1>我认为我们不那么正常的儿子可能正在经历一些典型的青少年叛逆。</S1>""",
        },
        ...
        # 2. biopsy
        {
            "name": "biopsy",
            "subject": (
                "Subject1: A woman with shoulder-length hair wearing a dark blazer over a light-colored top, "
                "seated at a table in an indoor setting with a colorful abstract painting on a wooden wall behind her, "
                "suggesting a formal or professional conversation."
            ),
            "narration_en": (
                "The overall environment sound is minimal. The woman initially faces the camera, then turns to her right "
                "and says <S1>From me. I had a biopsy taken from the smallpox vaccination scar.</S1> "
                "Her body language remains calm and focused, and she occasionally looks to her left and right as if "
                "explaining something or listening attentively."
            ),
            "narration_zh": (
                "The overall environment sound is minimal. The woman initially faces the camera, then turns to her right "
                "and says <S1>我从天花疫苗接种疤痕中做了活检。</S1> "
                "Her body language remains calm and focused, and she occasionally looks to her left and right as if "
                "explaining something or listening attentively."
            ),
            "text_en": """<S1>From me. I had a biopsy taken from the smallpox vaccination scar.</S1>""",
            "text_zh": """<S1>我从天花疫苗接种疤痕中做了活检。</S1>""",
        },
        ...
        # 3. grudge match
        {
            "name": "grudge_match",
            "subject": (
                "Subject1: A dark-haired woman facing left, wearing a black sweatshirt with blue trim, speaking outdoors "
                "in a forest setting. Subject2: A blond woman with braided hair in the foreground, wearing a brown top "
                "and listening intently amid soft green foliage."
            ),
            "narration_en": (
                "The overall environment sound is quiet with subtle ambient forest noises. "
                "The dark-haired woman has an attentive posture with her mouth open and her eyes fixed on the other party "
                "as she speaks, saying <S1>when we had that grudge match, over who had the shittiest life back home?</S1> "
                "The blond woman appears to be listening intently with her head slightly tilted toward the dark-haired woman."
            ),
            "narration_zh": (
                "The overall environment sound is quiet with subtle ambient forest noises. "
                "The dark-haired woman has an attentive posture with her mouth open and her eyes fixed on the other party "
                "as she speaks, saying <S1>是我们进行那次关于谁在老家过得最糟糕的比赛时吗？</S1> "
                "The blond woman appears to be listening intently with her head slightly tilted toward the dark-haired woman."
            ),
            "text_en": """<S1>when we had that grudge match, over who had the shittiest life back home?</S1>""",
            "text_zh": """<S1>是我们进行那次关于谁在老家过得最糟糕的比赛时吗？</S1>""",
        },
        ...
        # 4. job
        {
            "name": "job",
            "subject": (
                "Subject1: A middle-aged man with graying hair in a light gray suit jacket, white shirt, and yellow "
                "checked tie, seated in a dim office or interrogation room with brick walls and a grid-patterned window. "
                "Subject2: A woman with long dark hair in a dark gray suit jacket, seated beside him with her arms crossed, "
                "listening intently."
            ),
            "narration_en": (
                "The overall environment sound is quiet. The man holds a small notepad, looking down at it as he speaks, "
                "quoting someone. He says <S1>You said I needed a job. He gave me a job. Simple as that.</S1> "
                "The woman beside him listens intently with her arms crossed."
            ),
            "narration_zh": (
                "The overall environment sound is quiet. The man holds a small notepad, looking down at it as he speaks, "
                "quoting someone. He says <S1>你说我需要一份工作。他给了我一份工作。就这么简单。</S1> "
                "The woman beside him listens intently with her arms crossed."
            ),
            "text_en": """<S1>You said I needed a job. He gave me a job. Simple as that.</S1>""",
            "text_zh": """<S1>你说我需要一份工作。他给了我一份工作。就这么简单。</S1>""",
        },
        ...
        # 5. motorcycle
        {
            "name": "motorcycle",
            "subject": (
                "Subject1: A man in a gray suit jacket and patterned tie in a warmly lit restaurant or private club, "
                "facing a woman with long curly hair wearing a black outfit. Subject2: The woman in black, engaged in "
                "conversation amid other guests, a chandelier, and framed artwork."
            ),
            "narration_en": (
                "The overall environment sound is indistinct background chatter and ambient room noise. "
                "The man's right hand is raised to his chest, and he gestures with his index finger, moving from time "
                "to time during the conversation. He says <S1>I bought my motorcycle last year because you guys "
                "recommended it. You gave it five stars.</S1> The woman maintains eye contact and listens attentively, "
                "occasionally adjusting her posture to emphasize engagement."
            ),
            "narration_zh": (
                "The overall environment sound is indistinct background chatter and ambient room noise. "
                "The man's right hand is raised to his chest, and he gestures with his index finger, moving from time "
                "to time during the conversation. He says <S1>我去年买了我的摩托车，因为你们推荐了它。你们给它评了五星级。</S1> "
                "The woman maintains eye contact and listens attentively, occasionally adjusting her posture to emphasize "
                "engagement."
            ),
            "text_en": """<S1>I bought my motorcycle last year because you guys recommended it. You gave it five stars.</S1>""",
            "text_zh": """<S1>我去年买了我的摩托车，因为你们推荐了它。你们给它评了五星级。</S1>""",
        },

        # 6. I'm in a hurry + judge（两句）
        {
            "name": "in_a_hurry",
            "subject": (
                "Subject1: A bald man indoors wearing a dark jacket over a button-up shirt, standing in front of "
                "horizontal wooden paneling and a modern red lamp with a round lampshade that casts a warm glow."
            ),
            "narration_en": (
                "The overall environment sound is faint, indistinct background chatter. "
                "The man on screen, with a slightly concerned expression, says "
                "<S1>I'm in a hurry.</S1> An off-screen voice responds "
                "<S1>But it'll take at least a year before it makes it to the judge.</S1> "
                "The man on screen listens, his expression becoming more serious and contemplative."
            ),
            "narration_zh": (
                "The overall environment sound is faint, indistinct background chatter. "
                "The man on screen, with a slightly concerned expression, says "
                "<S1>我赶时间。</S1> An off-screen voice responds "
                "<S1>但至少需要一年时间才能到法官那里。</S1> "
                "The man on screen listens, his expression becoming more serious and contemplative."
            ),
            "text_en": """<S1>I'm in a hurry.</S1><S1>But it'll take at least a year before it makes it to the judge.</S1>""",
            "text_zh": """<S1>我赶时间。</S1><S1>但至少需要一年时间才能到法官那里。</S1>""",
        },
        ...
        # 7. cucumbers
        {
            "name": "cucumbers",
            "subject": (
                "Subject1: A woman with long dark hair in a greenhouse, wearing a blue and beige striped long-sleeved "
                "top and black pants, kneeling beside a row of soil with small green plants under a translucent "
                "plastic covering."
            ),
            "narration_en": (
                "The overall environment sound is minimal ambient sound. "
                "The woman kneels beside a row of small green plants. She explains something, gesturing with her hands, "
                "pointing at the plants or emphasizing certain points with her fingers, saying "
                "<S1>Okay, I'm finished with the cucumbers. I did go ahead and put some of the pickling cucumbers in.</S1> "
                "She then turns her head to the right of the frame to look at the plants and uses her left hand to gently "
                "touch or adjust them."
            ),
            "narration_zh": (
                "The overall environment sound is minimal ambient sound. "
                "The woman kneels beside a row of small green plants. She explains something, gesturing with her hands, "
            ...
            "text_en": """<S1>Okay, I'm finished with the cucumbers. I did go ahead and put some of the pickling cucumbers in.</S1>""",
            "text_zh": """<S1>好的，我已经处理完黄瓜了。我确实放了一些腌黄瓜进去。</S1>""",
        },

        # 8. traditional parents
        {
            "name": "traditional_parents",
            "subject": (
                "Subject1: A woman with long blond hair wearing a pink floral shirt, a pink headband, and earrings, "
                "standing in a warmly lit room decorated with children's drawings and a plaid shirt on a wooden wall. "
                "Subject2: A man in a green striped shirt standing opposite her with his back mostly to the camera."
            ),
            "narration_en": (
                "The overall environment sound is quiet. "
                "A woman with long blonde hair speaks with an animated and concerned expression to a man in a green striped shirt. "
                "She says <S1>You know how traditional my parents are. If they found out I got divorced and married to a black man, "
                "they'd crap in a sock.</S1> Her expression is intense as she talks."
            ),
            "narration_zh": (
                "The overall environment sound is quiet. "
                "A woman with long blonde hair speaks with an animated and concerned expression to a man in a green striped shirt. "
                "She says <S1>你知道我父母有多传统。如果他们发现我离婚并嫁给了一个黑人，他们会气死的。</S1> "
                "Her expression is intense as she talks."
            ),
            "text_en": """<S1>You know how traditional my parents are. If they found out I got divorced and married to a black man, they'd crap in a sock.</S1>""",
            "text_zh": """<S1>你知道我父母有多传统。如果他们发现我离婚并嫁给了一个黑人，他们会气死的。</S1>""",
        },
        ...
        # 9. security officer
        {
            ...
            "subject": (
                "Subject1: A man in a dark blue security-officer uniform over a white shirt, standing in a brightly lit "
                "indoor space with a yellow wall, a cuckoo clock, and a framed picture in the background."
            ),
            "narration_en": (
                "The overall environment sound is a quiet indoor ambiance. "
                "The man in the security uniform speaks, his facial expression changing subtly and his body moving slightly "
                "as he gestures with his hands to emphasize his words. He says "
            ...
            "text_en": """<S1>tracted by women with big chests. Um you know I might be looking at them or something but I I I</S1>""",
            "text_zh": """<S1>被漂亮的女性吸引。呃，你知道我可能正在看她们或者什么，但是我我我</S1>""",
        },

        # 10. oh no / he quit / mm-hmm
        {
            "name": "oh_no_he_quit",
            "subject": (
                "Subject1: A woman with long blond hair standing in a domestic living room, wearing a colorful printed "
                "blouse and a necklace, in front of a wooden cabinet with a flat-screen TV on top under soft, warm lighting."
            ),
            "narration_en": (
                "The overall environment sound is the clear speech of a woman. "
                "She starts speaking, gesturing with her arms bent at the waist, her palms facing her chest. "
                "She then stretches her arms outward and raises them up and down in an irregular manner, saying "
                "<S1>Oh, no. No, you know what? He's not into that stuff anymore.</S1> "
                "She continues, saying <S1>He quit for me.</S1> while lowering her arms and imitating holding a tennis ball "
                "in both hands. She then turns slightly to the right and looks, and finally she lowers her arms naturally "
                "and turns her head to look to the left side of the screen, adding <S1>Mm-hmm.</S1> "
                "The posture changes subtly, indicating a shift from explanation to relaxation."
            ),
            "narration_zh": (
                "The overall environment sound is the clear speech of a woman. "
                "She starts speaking, gesturing with her arms bent at the waist, her palms facing her chest. "
                "She then stretches her arms outward and raises them up and down in an irregular manner, saying "
                "<S1>哦，不。不，你知道的？他不再喜欢那类东西了。</S1> "
                "She continues, saying <S1>他为了我辞职了。</S1> while lowering her arms and imitating holding a tennis ball "
                "in both hands. She then turns slightly to the right and looks, and finally she lowers her arms naturally "
                "and turns her head to look to the left side of the screen, adding <S1>嗯嗯。</S1> "
                "The posture changes subtly, indicating a shift from explanation to relaxation."
            ),
            "text_en": """<S1>Oh, no. No, you know what? He's not into that stuff anymore.</S1><S1>He quit for me.</S1><S1>Mm-hmm.</S1>""",
            "text_zh": """<S1>哦，不。不，你知道的？他不再喜欢那类东西了。</S1><S1>他为了我辞职了。</S1><S1>嗯嗯。</S1>""",
        },
    ]

    # =====================================================================
    # 展开成 texts / captions（顺序：每组 EN 在前，ZH 在后）
    # =====================================================================
    texts = []
    captions = []
    for scene in scenes:
        # 英文
        texts.append(scene["text_en"])
        captions.append(
            "Subjects:" + scene["subject"] + " Narration:" + scene["narration_en"]
        )
        # 中文
        texts.append(scene["text_zh"])
        captions.append(
            "Subjects:" + scene["subject"] + " Narration:" + scene["narration_zh"]
        )
