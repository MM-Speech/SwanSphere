...
class Diffusion(nn.Module):
    ...
    def inference(self, inputs, timesteps=20, seq_cfg_w=[1.0, 1.5, 1.5], **kwargs):
        ...
        traj = torchdiffeq.odeint(
            ...
            torch.randn([1, tgt_len, self.hp.d_lat], device=device),
            t_schedule,
            atol=1e-4,
            rtol=1e-4,
            method="euler",
...
if __name__ == '__main__':
    ...
    inputs['global_clip_embedding'] = torch.randn([1, 20, 512], device=device)
    inputs['direction'] = torch.randn([1, 3], device=device)
    inputs['lat'] = torch.randn([1, 431, 288], device=device)
    ...
    with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
        ...
        ret = model.inference_cfg(inputs)
    print(f'{pred.shape =}')
    print(f'{ret.shape =}')
