...
class Diffusion(nn.Module):
    ...
    def forward(self, inputs, sigmas=None, x_noisy=None):
        ...
        if 'caption_emb' in inputs and inputs['caption_emb'] is not None and self.hp.use_caption_encoder:
            ...
            caption_embs = None; caption_lens = None
            
        if self.add_vad_mask:
            x_noisy = self.lat_proj(
                torch.cat([self.prenet(torch.cat([x_noisy, ctx_mask, inputs['vad_mask'][..., None]], -1)), x_txt], -1))
        else:
    ...
    def _forward(self, x, cond, timesteps, seq_cfg_w=(1.0, 1.5, 1.5, 1.5), timestep_annealing_w=(1.0, 0.0, 1.0)):
        ...
        x = x * (1 - ctx_mask) + ctx
        if self.add_vad_mask:
            x = self.lat_proj(torch.cat([self.prenet(torch.cat([x, ctx_mask, cond['vad_mask'][..., None]], -1)), x_txt], -1))
        else:
            x = self.lat_proj(torch.cat([self.prenet(torch.cat([x, ctx_mask], -1)), x_txt], -1))
        if 'caption_emb' in cond and cond['caption_emb'] is not None and self.hp.use_caption_encoder:
