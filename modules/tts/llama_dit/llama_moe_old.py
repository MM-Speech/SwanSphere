...
from modules.commons.attention import flash_attention
...
class ModelArgs:
    ...
    use_caption_pool_in_adaln: bool = True
...
class Attention(nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        self.wq = nn.Linear(args.encoder_dim, args.encoder_n_heads * self.head_dim)
        self.wk = nn.Linear(args.encoder_dim, self.encoder_n_kv_heads * self.head_dim)
        self.wv = nn.Linear(args.encoder_dim, self.encoder_n_kv_heads * self.head_dim)
        self.wo = nn.Linear(args.encoder_n_heads * self.head_dim, args.encoder_dim)
    ...
    def forward(
        ...
        if use_cache:
            ...
            query_states = xq.transpose(1, 2)   # (bs, seqlen, n_local_heads, head_dim)
            attention_mask = mask
            key_states = keys.transpose(1, 2)
            value_states = values.transpose(1, 2)

            output = flash_attn_with_kvcache(
                query_states, self.cache_k, self.cache_v, key_states, value_states,
                cache_seqlens=start_pos, causal=self.use_causal_attn
            )
            output = output.contiguous().view(bsz, seqlen, -1)

        else:
            if self.use_normal_attn:
                # 与“上面代码”一致：直接将 mask 传入 SDPA（若为 float 类型则为加性 mask；若为 bool 则 True 表示屏蔽）
                output = F.scaled_dot_product_attention(
                    xq, keys, values, mask[:, None, None, :],
                    is_causal=self.use_causal_attn
                )
                ...
                # flash-attn varlen 分支（与上面代码一致）
                query_states = xq.transpose(1, 2)   # (bs, seqlen, n_local_heads, head_dim)
                attention_mask = mask               # [B, T]，通常为 bool，True=有效
                key_states = keys.transpose(1, 2)
                value_states = values.transpose(1, 2)
    ...
    def _upad_input(self, query_layer, key_layer, value_layer, attention_mask, query_length):
        ...
        if query_length == kv_seq_len:
            ...
            attention_mask = attention_mask[:, -query_length:]  # left padding
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context, context_lens, query_lens=None, query_mask=None):
        ...
        # 与“上面代码”一致的 qmask 构造：优先用 query_lens，否则全 1（有效）
...
def cross_attn_varlen(q, k, v, query_mask, context_mask):
    indices_q, cu_seqlens_q, max_seqlen_q = _get_unpad_data(query_mask)
    indices_k, cu_seqlens_k, max_seqlen_k = _get_unpad_data(context_mask)
...
class TransformerBlock(nn.Module):
    def __init__(self, args: ModelArgs, enable_crossattn=True):
        super().__init__()
        self.args = args
        ...
        # 新增 audio-path FFN（严格随机初始化，不拷贝任何已有权重）
        self.feed_forward_audio = FeedForward(
            dim=args.encoder_dim,
            hidden_dim=args.encoder_dim,
            multiple_of=args.multiple_of,
            ffn_dim_multiplier=args.ffn_dim_multiplier,
    ...
    def forward(
        ...
        if self.enable_crossattn and context is not None:
            ...
            h = h + self.cross_gate.to(dtype) * cross_attn_output  # gated residual

        # === FFN（双路）：严格保留双 FFN，其余与上面代码一致 ===
        norm = self.ffn_norm(h.float()).to(dtype) * (1 + scale_mlp[:, None]) + shift_mlp[:, None]
        ffn_speech = self.feed_forward(norm)
        ffn_audio  = self.feed_forward_audio(norm)
        ffn_out = ffn_speech + ffn_audio

        out = h + gate_mlp.unsqueeze(1) * ffn_out
        return out
...
class LLaMa(nn.Module):
    ...
    def forward(
        ...
        """
        若 params.use_caption_pool_in_adaln=True：
          - 按 context_lens 对 context 做 mean-pool 得到 cap_pool
          - 过一层 cap_embedder 投影：cap_emb = cap_embedder(cap_pool)
          - 与时间步嵌入相加：t_mod = t + cap_emb
          - 用 t_mod 作为 AdaLN 的调制向量（逐层 & 最后）
        否则：
          - 维持旧行为，仅用 t 调制。
        """
        # 取本段需要的 RoPE 相位
        ...
        # 可选：caption 全局池化 + 线性投影 + 融合到 AdaLN 调制（严格按第二版）
        if self._use_cap_mod and (context is not None) and (context_lens is not None):
            ...
            cap_emb = self.cap_embedder(cap_pool)                                  # [B, C]
            t_mod = t + cap_emb.to(t.dtype)
        ...
        for i, layer in enumerate(self.layers):
            if torch.is_grad_enabled() and do_checkpoint:
                ...
                x = torch.utils.checkpoint.checkpoint(
                    ...
                    x, t_mod, start_pos, freqs_cis, attn_mask, context, context_lens, use_cache, **ckpt_kwargs,
                )
            else:
                x = layer(
                    x, t_mod, start_pos, freqs_cis, attn_mask,
                    context=context, context_lens=context_lens, use_cache=use_cache
                )

        # 末端 AdaLN 也用 t_mod（或 t）
        x = self.norm(x, t_mod)
        x = self.out_proj(x)
        return x
