...
class Attention(nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        if self.use_gated_attention:
            self.attn_gate = nn.Linear(args.encoder_dim, args.encoder_n_heads * self.head_dim, bias=False)
    ...
    def forward(
        ...
        if use_cache:
            ...
            if self.use_sdpa:
                add_mask = make_additive_mask_from_padding(mask, q_len=seqlen, device=x.device, dtype=torch.float32)
                output = F.scaled_dot_product_attention(
                    xq, keys, values,
                    attn_mask=add_mask, is_causal=False
                )
        ...
        if self.use_gated_attention:
            return self.wo(output * torch.sigmoid(self.attn_gate(x)))
        else:
...
class TransformerBlock(nn.Module):
    ...
    def forward(
        ...
        if self.enable_crossattn and context is not None:
            norm = self.cross_attention_norm(h.float()).to(dtype)   # [B, T, C]
            ...
            if self.args.use_dynamic_cross_gate:
                ...
                # print(f"{h.mean() = }, {cross_gate.mean() = }, {cross_attn_output.mean() = }")
...
class LLaMa(nn.Module):
    def __init__(self, params: ModelArgs):
        ...
        if not hasattr(params, "use_gated_attention"):
            params.use_gated_attention = False
        ...
        if self._use_cap_mod:
            ...
            if getattr(self.params, 'use_caption_pool_gate_in_adaln', False):
    ...
    def forward(self, x, t, attn_mask, context=None, context_lens=None, start_pos=0, use_cache=False, do_checkpoint=False):
        ...
        for i, layer in enumerate(self.layers):
            if torch.is_grad_enabled() and do_checkpoint:
                ...
                x = torch.utils.checkpoint.checkpoint(
                    ...
                    x, t, start_pos, freqs_cis, attn_mask, context, context_lens, use_cache, **ckpt_kwargs,
                )

            else:
                x = layer(x, t, start_pos, freqs_cis, attn_mask, context=context, context_lens=context_lens, use_cache=use_cache)
