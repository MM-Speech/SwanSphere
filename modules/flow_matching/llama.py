...
def merge_sequence(seq_1, seq_1_len, seq_2, seq_2_len, dtype=torch.float16):
    ...
    B, C, device, T_seq_1, T_seq_2, T_merged = seq_1.size(0), seq_1.size(-1), seq_1.device, seq_1.size(1), seq_2.size(
        1), (seq_1_len + seq_2_len).max()
    ...
    mask_for_loss = mask_x_2 = (x_indices >= seq_1_len[:, None]) & (x_indices < (seq_1_len + seq_2_len)[:, None]) & (
            x_indices - seq_1_len[:, None] < T_seq_2)
...
class Attention(nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        try:
            ...
            if use_qk_norm:
                self.q_norm = RMSNormFp32(self.head_dim, eps=1e-8)
                self.k_norm = RMSNormFp32(self.head_dim, eps=1e-8)
        except:
            traceback.print_exc()
    ...
    def forward(
        ...
        # print('attn_output_unpad', attn_output_unpad)
...
class TransformerBlock(nn.Module):
    ...
    def forward(
        ...
        out = h + self.feed_forward(
            self.ffn_norm(h.to(torch.float32)).to(h_dtype)
        )
        out = out.to(h_dtype)
        return out
...
class LLaMa(nn.Module):
    ...
    def forward(self, x, attn_mask):
        start_pos = 0
        freqs_cis = torch.view_as_complex(self.freqs_cis.to(dtype=torch.float32))[start_pos: start_pos + x.size(1)].to(x.dtype)
        for layer in self.layers:
            x = layer(x, start_pos, freqs_cis, attn_mask)
        x = self.norm(x)
        x = self.out_proj(x)
        return x
