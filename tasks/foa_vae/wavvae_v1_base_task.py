...
class FOAWavVAEBaseTask(FastDatasetMixin, BaseTask):
    ...
    def build_model(self):
        init_pretrained = (
            self.stage_name == "projector_warmup"
    ...
    def _apply_build_time_trainability(self):
        if self.stage_cfg.get("trainable", "full") == "adapter":
    ...
    def build_scheduler(self, optimizer):
        ...
        if scheduler_cfg.get("enabled", False):
            ...
            generator_scheduler = CosineAnnealingWarmRestartsWithWarmup(
                optimizer[0],
            ...
            return (generator_scheduler, None)
        return (None, None)
    ...
    def _apply_trainability(self, optimizer_idx):
        ...
        if mode == "adapter":
            from modules.foa_vae.wavvae_v1 import adapter_param_prefixes
