...
from modules.foa_vae.wavvae_v3_post import FOAV3SpatialLoss
from tasks.foa_vae.wavvae_v3_task import FOAWavVAEV3Task, trim_to_shortest
from utils.commons.hparams import hparams


class FOAWavVAEV3PostTask(FOAWavVAEV3Task):
    """V3 training with additional fixed-weight FOA spatial losses."""
    ...
    def build_model(self):
        ...
        self.spatial_post_loss = FOAV3SpatialLoss(
    ...
    def _add_spatial_losses(self, reconstruction, target, loss_output):
        ...
        if self.spatial_post_loss is None:
            raise RuntimeError("FOA V3 post spatial loss is enabled but not initialized")
    ...
    def _training_step(self, sample, batch_idx, optimizer_idx):
        ...
        with torch.autocast(device_type=autocast_device, enabled=False):
            self._add_spatial_losses(reconstruction.float(), target.float(), loss_output)
            ...
            if lambda_adv or lambda_feature_matching:
                with torch.autocast(device_type=autocast_device, enabled=False):
                    loss_adv, feature_matching = self._disc_generator_loss(
                        target.float(), reconstruction.float()
                    )
                if lambda_adv:
        ...
        loss_terms = [value for key, value in loss_output.items() if not key.startswith("monitor/")]
        if not loss_terms:
            return None
