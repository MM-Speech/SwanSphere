...
class PromptAudioTask(DiTBuildModelMixinV2, FastDatasetDiTBaseTask):
    ...
    def run_caption_encoder(self, captions, device):
        ...
        return encoder_hidden_states, attention_masks,None
    ...
    def run_model(self, sample, infer=False, infer_steps=None):
        ...
        loss_mask = sequence_mask(lat_lens)[:, :, None] 
        ...
        ctx_mask = torch.zeros(bsz, T_lat, device=device, dtype=lat.dtype)  # [B, T]
        inputs = {
            "phone": ph_tokens,
            "tone": tone_tokens,
            ...
            # "mel2ph": mel2ph,            
            "lat_ctx": lat_ctx,                # 保留键名
            "ctx_mask": ctx_mask[:, :, None],  # [B,T,1] 全零
        }
...
class PromptAudioDiTV4Task(DiTBuildModelMixinV4, PromptAudioTask):
    pass
