...
def processer_fn_spatial(raw_item, tgt_size, hparams, global_stores, skip_logger, i_worker, n_worker):
    ...
    processer_args = ProcesserArgs(
        ...
        energy_map_path = hparams['energy_map_path'],
        raw_opus_base_path = hparams['raw_opus_base_path'],
        # 剩下的怎么赋值？
    )
    ...
    for _item in raw_item:
        ...
        max_second = int(dac.shape[0] // processer_args.dac_frame_rate)
        if max_second > processer_args.seconds_to_use:
            ...
            dac = dac[
                start_second
                * processer_args.dac_frame_rate : (start_second + processer_args.seconds_to_use)
                * processer_args.dac_frame_rate
            ]
            dac_z = dac_z[
                start_second
                * processer_args.dac_frame_rate : (start_second + processer_args.seconds_to_use)
            ...
            dac_pad = processer_args.seconds_to_use * processer_args.dac_frame_rate - dac.shape[0]
            if dac_pad > 0:
                dac = np.pad(
                    dac,
                    ((0, dac_pad), (0, 0)),
                ...
                dac = dac[: processer_args.seconds_to_use * processer_args.dac_frame_rate]
                dac_z = dac_z[: processer_args.seconds_to_use * processer_args.dac_frame_rate]
        ...
        if dac.shape[0] != processer_args.dac_frame_rate * processer_args.seconds_to_use:
...
#     hparams = AttrDict({
#         'dac_base_path':        '/mnt/bn/sa-ag-data/leike/spatial/data/YT-Ambigen/dac',
#         'dac_z_base_path':        '/mnt/bn/sa-ag-data/leike/spatial/data/YT-Ambigen/dac_z',
#         'clip_base_path':       '/mnt/bn/sa-ag-data/leike/spatial/data/YT-Ambigen/clip',
...
#     raw_item = [{'file_name': '-RYuQTqNySE_8.npy'},
#                 {'file_name': '-RYuQTqNySE_13.npy'}]
